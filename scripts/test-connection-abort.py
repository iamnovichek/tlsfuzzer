...
def main():
    ...
    print("Test server behaviour when the client aborts connection at".upper())
    print("different times.".upper())
    print("For TLS 1.2 or earlier and RSA key exchange (or (EC)DHE if".upper())
    ...
    conversations["After receiving ApplicationData"] = conversation
    ...
    node = node.add_child(ApplicationDataGenerator(
        bytearray(b"GET / HTTP/1.0\r\n\r\n")))
    node = node.add_child(Close())
    conversations["After sending ApplicationData"] = conversation
    ...
    node = node.add_child(ExpectChangeCipherSpec())
    node = node.add_child(Close())
    conversations["After server CCS"] = conversation
    ...
    node = node.add_child(ChangeCipherSpecGenerator())
    node = node.add_child(Close())
    conversations["After client CCS"] = conversation
    ...
    node = node.add_child(ClientKeyExchangeGenerator())
    node = node.add_child(Close())
    conversations["After ClientKeyExchange"] = conversation
    ...
    node = node.add_child(ExpectServerHelloDone())
    node = node.add_child(Close())
    conversations["After ServerHelloDone"] = conversation
    ...
    if dhe:
        ...
        node = node.add_child(ExpectServerKeyExchange())
        node = node.add_child(Close())
        conversations["After ServerKeyExchange"] = conversation
