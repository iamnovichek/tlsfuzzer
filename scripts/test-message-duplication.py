...
def main():
    ...
    print("Test message duplication".upper())
    print("=" * CHARACTERS_LENGTH)
    time.sleep(3)

    argv = sys.argv[1:]
    opts, args = getopt.getopt(argv, "h:p:n:e:x:X", ["help"])
    for opt, arg in opts:
        if opt == '-h':
            host = arg
        elif opt == '-p':
    ...
    node = node.add_child(ExpectAlert(AlertLevel.fatal,
                                      AlertDescription.unexpected_message))
    node = node.add_child(ExpectClose())
    conversations["duplicate client key exchange before Finished"] = conversation
    ...
    node = node.add_child(ExpectAlert(AlertLevel.fatal,
                                      AlertDescription.unexpected_message))
    node = node.add_child(ExpectClose())
    conversations["duplicate change cipher spec before Finished"] = conversation
